...
const YB6_CLASSIC_CONTRACTS = CONTRACTS.filter(contract =>
  contract.from === 'yarn-berry-v6' && contract.to === 'yarn-classic',
) as ConversionContract[]

describe('interop: yarn-berry-v6 -> yarn-classic (cross-family)', () => {
  for (const contract of YB6_CLASSIC_CONTRACTS) {
    describe(`${contract.from} -> ${contract.to}`, () => {
      it.each(CLASSIC_SHARED_FIXTURES)('%s fixture satisfies the declared contract', fixtureName => {
        const source = classicFixtureAsBerrySource(fixtureName, 'yarn-berry-v6')
