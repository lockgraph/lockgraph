...
// Workspace-focused berry corpus for v5/v6/v8/v9 pairs: derives
...
// patchedDependencies / overrides:patch primitive per -v5).
...
// Cross-family yarn-berry-v9 <-> npm-3 shared corpus (Phase C-iii):
...
// Cross-family yarn-berry-v4 <-> npm-3 shared corpus (Phase E-i):
...
// Cross-family yarn-berry-v{5,6,8} <-> npm-3 shared corpus (Phase
...
// FORWARD-only (yarn-berry -> npm-3) corpora narrow
...
// Cross-family pnpm-v9 <-> npm-3 shared corpus (Phase C-iv): fixtures
...
// Cross-family pnpm-v5 <-> npm-3 shared corpus (Phase E-iv): same
...
// Cross-family pnpm-v6 <-> npm-3 shared corpus (Phase E-iv): same
...
// Cross-family yarn-classic -> pnpm-v9 corpus (Phase D-i): start
...
// disk (no root/member workspace nodes), so cross-family
...
// Cross-family yarn-classic -> pnpm-v5 corpus (Phase E-iv): mirrors
...
// Cross-family yarn-classic -> pnpm-v6 corpus (Phase E-iv): same
...
// Cross-family pnpm-v9 -> yarn-classic corpus (Phase D-i): start
...
// Cross-family pnpm-v5 -> yarn-classic corpus (Phase E-iv): same
...
// Cross-family pnpm-v6 -> yarn-classic corpus (Phase E-iv): mirrors
...
// Cross-family yarn-classic -> npm-3 corpus (Phase D-ii): start
...
//). npm-3 now preserves that package and its tarball beneath
...
// Cross-family npm-3 -> yarn-classic corpus (Phase D-ii): start
...
// Cross-family npm-3 <-> bun-text shared corpus (Phase C-vii):
...
// `npm-3.lock` is absent and bun-text has no patch primitive;
// `workspace-cross-refs` excluded because `npm-3.lock` is absent.
//
...
// Cross-family yarn-classic <-> bun-text shared corpus (Phase D-iii):
...
// Cross-family yarn-berry-v4 <-> bun-text shared corpus (Phase
...
// Cross-family yarn-berry-v{5,6,8} <-> bun-text shared corpus (Phase
...
// Cross-family yarn-berry-v9 <-> bun-text shared corpus (Phase C-v):
// fixtures with both `yarn-berry-v9.lock` AND `bun-text.lock` on disk.
// Exclusions vs the 7-fixture intersection:
// `patch-yarn`: bun-text has no `patch:` primitive (— bun-text
...
// the loss honestly; first-class cross-family patch
...
// Cross-family pnpm-v9 <-> bun-text shared corpus (Phase C-vi):
...
// patch primitive. The forward adapter already emits
...
// Cross-family pnpm-v5 <-> bun-text shared corpus (Phase E-iv): same
...
// Cross-family pnpm-v6 <-> bun-text shared corpus (Phase E-iv):
...
// Cross-family yarn-berry-v9 <-> pnpm-v9 shared corpus (Phase C-i):
...
// loss (sentinel hash differs across PMs by canonical input per — yarn
// hashes the patch locator, pnpm hashes `<name>@<version>:<literalKey>`) on top
// of the workspace-rekey + tarball-extras losses. Per honesty
...
// Cross-family yarn-berry-v9 <-> pnpm-v5 shared corpus (Phase E-iv):
...
// Cross-family yarn-berry-v9 <-> pnpm-v6 shared corpus (Phase E-iv):
...
// Cross-family yarn-berry-v4 <-> pnpm-v9 shared corpus (Phase E-i):
...
// (Phase E-ii): fixtures with all three mid-berry lockfiles plus
...
// Cross-family npm-1 <-> yarn-berry-v9 shared corpus (Phase E-v):
...
// does not preserve the workspace-member graph honestly (-1)
...
// Cross-family npm-1 <-> pnpm-v9 shared corpus (Phase E-v): same
...
// Cross-family npm-1 <-> bun-text shared corpus (Phase E-v): start
// from the 6-fixture disk intersection and EXCLUDE npm-1-unsafe
// workspace-bearing fixtures:
// `peers-multi`, `workspaces-basic`: excluded per -1
// - `git-github-tarball`: no `bun-text.lock` on disk
...
// Cross-family npm-1 <-> yarn-classic shared corpus (Phase E-v):
