...
// integrity is held canonical (`sha512-<base64>` SRI) on the
...
// `INTEROP_..._INTEGRITY_PASSTHROUGH`.
...
function buildClassicToBerry(to: BerryFormat): ConversionContract {
  ...
  // canonical SRI integrity preserved across all berry
...
function buildBerryToClassic(from: BerryFormat): ConversionContract {
  ...
  const lost: LossEntry[] = [
    ...
    },
    {
      ...
      severity: 'warning',
      rationale: 'classic has no slot-value namespace, so unresolved- sentinel patches () collapse on emit',
    },
    {
      feature: 'virtual',
...
// === npm intra-family contracts (Phase A) ==========================
//
// 6 directional pairs across npm-1 / npm-2 / npm-3. Shape deltas:
//
...
// npm-3 packages-only (no legacy `dependencies` mirror). Workspace
// `link: true`.
//
// Graph-level loss analysis (ConversionContract derivation):
//
...
// is not part of the npm-1 entry schema (-1);
...
// on emit (-1, npm v6 dialect); reparse forms a NodeId
...
function npmToNpm1LossEntries(from: NpmFormat): LossEntry[] {
  ...
  return [
    ...
    // per honesty principle. Peer-edge drop is already covered
...
function buildNpmIntraPair(from: NpmFormat, to: NpmFormat): ConversionContract {
  ...
  // diagnostics, instead of communicating loss only
  ...
  // npm-1 → npm-{2,3}: lossless-reentrant (npm-1 source is feature-poor
...
// === pnpm intra-family contracts (Phase B) =========================
//
// 6 directional pairs across pnpm-v5 / pnpm-v6 / pnpm-v9. Shape deltas
//
...
// patch supported via overrides:patch.
...
// sidecar bridge is absent — v5 owns a private WeakMap
...
// primitive (-v5), so `Node.patch` is dropped
...
//
// Settings drop (v5 has no settings block per -v5) is
...
function pnpmV5CrossLossEntries(from: PnpmFormat, to: PnpmFormat): LossEntry[] {
  ...
  if (to === 'pnpm-v5') {
    entries.push({
      ...
      severity: 'warning',
      rationale: 'pnpm-v5 has no patchedDependencies / overrides:patch primitive (-v5); Node.patch attribute drops on emit and the patched-id TarballKey collapses to bare locator',
    })
  }
  return entries
}
...
// === cross-family yarn-berry-v9 <-> pnpm-v9 (Phase C-i) ============
//
...
// sentinel nodes lack it (different input string),
...
// cross-family patch coverage deferred — narrow-rather-than-overdeclare
// Phase A precedent.
...
// pnpm-v9 -> yarn-berry-v9 (Phase C-ii reverse direction).
...
// drops on those fixtures — the parenthesized NodeId is unreachable
...
// === cross-family yarn-berry-v4 <-> pnpm-v9 (Phase E-i) ============
//
...
// === cross-family yarn-berry-v{5,6,8} <-> pnpm-v9 (Phase E-ii) ====
//
...
// === cross-family yarn-berry-v9 <-> pnpm-v{5,6} (Phase E-iv) ======
//
...
// === cross-family yarn-berry-v9 <-> npm-3 (Phase C-iii) ============
//
...
// === cross-family yarn-berry-v4 <-> npm-3 (Phase E-i) ==============
//
    ...
    lost: [
      {
        ...
        severity:   'warning',
        rationale:  'yarn-berry-v4 stringifier emits the PM-native `<name>@<locator>` envelope into npm-3 `resolved:` fields; the npm-3 parser cannot translate that back to canonical registry resolution, so `tarball.resolution.type` degrades to `unknown` (raw locator preserved as attribution). Distinct from yb4 <-> pnpm-v9 where the apparent resolved-url loss is subsumed under workspace-rekey; here workspace identity aligns (yb4 + npm-3 both use `<name>@0.0.0-use.local` for workspace members). : `git-github-tarball` is NARROWED OUT of the forward corpus — a git source carries a `+src=` NodeId discriminator on the yarn-berry side that the npm-3 reparse (canonical `unknown` -> BARE) cannot reproduce, an irreducible identity divergence narrowed rather than declared (mirrors NPM_TO_NPM_1_EXCLUDED). The drop fires on the remaining tarball-bearing registry nodes.',
      },
    ],
    added:       [],
    passthrough: [],
...
// === cross-family yarn-berry-v{5,6,8} <-> npm-3 (Phase E-ii) =======
//
    ...
    lost: [
      {
        ...
        severity:   'warning',
        rationale:  `yarn-berry-v${version} stringifier emits the PM-native \`<name>@<locator>\` envelope into npm-3 \`resolved:\` fields; the npm-3 parser cannot translate that back to canonical registry resolution, so \`tarball.resolution.type\` degrades to \`unknown\` (raw locator preserved as attribution). Distinct from yb-v${version} <-> pnpm-v9 where the apparent resolved-url loss is subsumed under workspace-rekey; here workspace identity aligns (yb-v${version} + npm-3 both use \`<name>@0.0.0-use.local\` for workspace members). : \`git-github-tarball\` is NARROWED OUT of the forward corpus — a git source carries a \`+src=\` NodeId discriminator on the yarn-berry side that npm-3 reparse (canonical \`unknown\` -> BARE) cannot reproduce, an irreducible identity divergence narrowed rather than declared (mirrors NPM_TO_NPM_1_EXCLUDED). The drop fires on the remaining tarball-bearing registry nodes.`,
      },
    ],
    added:       [],
    passthrough: [],
...
// === cross-family pnpm-v9 <-> npm-3 (Phase C-iv) ===================
//
...
// 3 — npm has no parenthesized peer-context
...
// peer-virt to begin with (npm flattens peer-context on parse
...
// === cross-family pnpm-v{5,6} <-> npm-3 (Phase E-iv) ==============
//
    ...
    lost: [
      {
        ...
        severity:   'warning',
        rationale:  `npm-3 stringify flattens peer-virt NodeIds via \`NPM_V3_PEER_VIRT_FLATTENED\` (-3): pnpm-v${version} source \`<name>@<version>(<peer-ctx>)\` collapses to bare \`<name>@<version>\` in destination. Source nodes / edges / edge-kinds / integrity / resolved-url tied to the parenthesized NodeId become unreachable in destination by id — peer-virt loss subsumes those graph-shape drops on peer-virt-bearing fixtures (peers-basic, peers-multi).`,
      },
    ],
    added:        [],
    passthrough:  [],
...
// === cross-family yarn-classic <-> pnpm-v9 (Phase D-i) ============
//
...
// workspace nodes), so pnpm-v9 stringify synthesises
...
// === cross-family yarn-classic <-> pnpm-v{5,6} (Phase E-iv) =======
//
...
// === cross-family yarn-classic <-> npm-3 (Phase D-ii) =============
//
// Empirically-probed loss profile:
//
...
// === cross-family yarn-classic <-> bun-text (Phase D-iii) ==========
//
...
// workspace nodes), so bun-text reparses that lone node
...
// === cross-family yarn-berry-v9 <-> bun-text (Phase C-v) ===========
//
...
// `patch-yarn` excluded — bun-text has no patch primitive,
...
// the integrity slot, NOT the URL (stringify table —
...
// `bin` only if the parse-time sidecar carried one (line 992
...
// Already declared at the adapter layer; NOT lifted к
...
// === cross-family yarn-berry-v{4,5,6,8} <-> bun-text (Phase E-iii) =
//
...
// === cross-family pnpm-v9 <-> bun-text (Phase C-vi) ================
//
...
// the multi-stack analogue) flattens to bare `react-dom@...`, which
...
// remains adapter-layer only per existing / precedent.
//
...
// === cross-family pnpm-v{5,6} <-> bun-text (Phase E-iv) ===========
//
...
// === cross-family npm-3 <-> bun-text (Phase C-vii) =================
//
...
// `@case-peers-multi/b` workspace edges onto the react@17 stack instead
// the source-declared react@18 stack and rewires `react-dom@18.2.0`
...
// loss. Per honesty principle: narrow the corpus, track the
// blocker separately.
//
...
// === cross-family npm-{1,2} <-> yarn-berry-v9 (Phase E-v) =========
//
...
// the workspace-member graph on disk, so they stay out
...
// === cross-family npm-{1,2} <-> pnpm-v9 (Phase E-v) ===============
//
...
// === cross-family npm-{1,2} <-> bun-text (Phase E-v) ==============
//
...
// === cross-family npm-{1,2} <-> yarn-classic (Phase E-v) ==========
//
...
// TODO(adr-0020): explicitly deferred compressionLevel, but the
