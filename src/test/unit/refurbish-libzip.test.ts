...
// The installed libzip reproduces only its own generation, so refurbish
// CALIBRATES against one existing sibling checksum before trusting a fill.
...
describe('enrich/refurbish — optional @yarnpkg/libzip backend (cacheKey 9/10, calibrated)', () => {
  it.skipIf(!hasLibzip)('calibrates against a sibling checksum, then fills the gap via libzip', async () => {
    // `ms` carries the REAL cacheKey-10 checksum (the calibration anchor);
    // `is-buffer` is the gap. cacheKey 10 is NOT pako-reproducible.
  ...
  it.skipIf(!hasLibzip)('defers when calibration fails (installed libzip ≠ lock generation)', async () => {
    // The anchor carries a WRONG checksum → the installed libzip's reproduction
    // won't match → refurbish must NOT trust it, and the gap defers.
    const graph = graphOf(b => {
      addPackage(b, { name: 'ms',        version: '2.1.3' })
      addPackage(b, { name: 'is-buffer', version: '2.0.5' })
