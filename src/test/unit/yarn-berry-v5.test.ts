...
function parseFixtureGraph(name: typeof FIXTURES[number]): Graph {
  return parseV5(fixture(`${name}/yarn-berry-v5.lock`))
}

function stringifyWithDiagnostics(graph: Graph) {
  const diagnostics: Diagnostic[] = []
  const lockfile = stringifyV5(graph, {
...
describe('yarn-berry-v5 — discriminant and isolation', () => {
  it('accepts v5 lockfile header and rejects v6/v8/v9 headers', () => {
    expect(checkV5(fixture('simple/yarn-berry-v5.lock'))).toBe(true)
    expect(checkV5(fixture('simple/yarn-berry-v6.lock'))).toBe(false)
    expect(checkV5(fixture('simple/yarn-berry-v8.lock'))).toBe(false)
    expect(checkV5(fixture('simple/yarn-berry-v9.lock'))).toBe(false)
    expect(checkV6(fixture('simple/yarn-berry-v5.lock'))).toBe(false)
    expect(checkV8(fixture('simple/yarn-berry-v5.lock'))).toBe(false)
    expect(checkV9(fixture('simple/yarn-berry-v5.lock'))).toBe(false)
  })
  ...
  it('parses with the matching adapter and rejects mismatched lockfileVersion', () => {
    ...
    for (const lock of [
      ...
      expect(() => parseV5(lock)).toThrow(LockfileError)
      try {
        parseV5(lock)
      } catch (error) {
...
describe('yarn-berry-v5 — parse fixtures', () => {
...
describe('yarn-berry-v5 — stringify', () => {
  it.each(FIXTURES.filter(name => name !== 'yarn-crlf'))('roundtrips %s at Graph level', (fixtureName) => {
    ...
    const emitted = stringifyV5(original)
    const reparsed = parseV5(emitted)
  ...
  it('roundtrips yarn-crlf at Graph level when CRLF is requested', () => {
    ...
    const emitted = stringifyV5(original, { lineEnding: 'crlf' })
    const reparsed = parseV5(emitted)
...
describe('yarn-berry-v5 — modify', () => {
  ...
  it('roundtrips replacePeerContext to the flattened graph with a v5 warning', () => {
    ...
    const reparsed = parseV5(lockfile)

    expectEmptyGraphDiff(original.diff(reparsed))
    expect(diagnostics).toEqual(expect.arrayContaining([
      expect.objectContaining({
        code: 'YARN_BERRY_V5_PEER_VIRT_FLATTENED',
        severity: 'warning',
  ...
  it('roundtrips setTarball', () => {
    ...
    const emitted = stringifyV5(result.graph)
    const reparsed = parseV5(emitted)

    expectEmptyGraphDiff(result.graph.diff(reparsed))
    expect(emitted).toContain(`checksum: ${MODIFIED_HEX}`)
    expect(emitted).not.toContain(`checksum: 8/${MODIFIED_HEX}`)
    // ADR-0014 §4.F3 — round-trip parse re-derives canonical resolution; the
    // integrity is the mutator's payload field, which is what we assert.
...
describe('yarn-berry-v5 — enrich', () => {
  it('derives a peer edge and virtualizes the consumer when one candidate matches', () => {
    ...
    const result = enrichV5(parseV5(input))
  ...
  it('warns when a peer range is ambiguous', () => {
    ...
    expect(enrichV5(parseV5(input)).diagnostics).toEqual([
      {
        code: 'YARN_BERRY_V5_PEER_AMBIGUOUS',
        severity: 'warning',
        subject: 'host@1.0.0',
  ...
  it('warns when a peer range is unsatisfied', () => {
    ...
    expect(enrichV5(parseV5(input)).diagnostics).toEqual([
      {
        code: 'YARN_BERRY_V5_PEER_UNSATISFIED',
        severity: 'warning',
        subject: 'host@1.0.0',
  ...
  it('throws INVALID_INPUT on a malformed peer range', () => {
    ...
    expect(() => enrichV5(parseV5(input))).toThrow(LockfileError)
    try {
      enrichV5(parseV5(input))
    } catch (error) {
...
describe('yarn-berry-v5 — optimize', () => {
  ...
  it('prunes unreachable nodes and tarballs', () => {
    const result = optimizeV5(graphWithOrphan())
  ...
  it('is idempotent across stringify/parse', () => {
    const once = optimizeV5(graphWithOrphan())
    const twice = optimizeV5(parseV5(stringifyV5(once.graph)))
